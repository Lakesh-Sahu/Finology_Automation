...
//    }
    ...
    public void user_is_on_new_sign_up_page() {
        Assert.assertTrue(ObjectManager.getObject().signUpPage.goToSignUpPage(), "User is not on sign up page");
    }

    @Then("User should be redirected to sign up page")
    public void user_should_be_redirected_to_sign_up_page() {
        Assert.assertTrue(ObjectManager.getObject().signUpPage.verifyOnSignUpPage(), "User is not redirected to sign up page");
    }

    @Given("User enters {string}, {string} and {string}")
    public void user_enters_and(String name, String emailId, String mobileNumber) {
        Assert.assertTrue(ObjectManager.getObject().signUpPage.enterSignUpDetails(name, emailId, mobileNumber), "User is unable to enter sign up details");
    }

    @When("User clicks sign up button")
    public void user_clicks_sign_up_button() {
        Assert.assertTrue(ObjectManager.getObject().signUpPage.clickSignUpBtn(), "User is unable to click signup button in signup page");
    }
    ...
    public void invalid_name_error_should_be_shown_below_name_input_box(String message) {
        Assert.assertTrue(compareString(ObjectManager.getObject().signUpPage.getInvalidNameErrorMessage(), message), "Invalid name error message does not match");
    }
    ...
    public void empty_name_error_should_be_shown_below_name_input_box(String message) {
        Assert.assertTrue(compareString(ObjectManager.getObject().signUpPage.getEmptyNameErrorMessage(), message), "Empty name error message does not match");
    }
    ...
    public void error_should_be_shown_below_email_input_box(String message1, String message2) {
        Assert.assertTrue(compareString(ObjectManager.getObject().signUpPage.getInvalidEmailErrorMessage(), message1) || compareString(ObjectManager.getObject().signUpPage.getInvalidEmailErrorMessage(), message2), "Invalid Email error message does not match");
    }
    ...
    public void user_should_see_incognito_mode_error_message_or_redirected_to_OTP_verification_page(String message) {
        Assert.assertTrue(compareString(ObjectManager.getObject().signUpPage.getIncognitoModeErrorMessage(), message) || ObjectManager.getObject().otpVerificationPage.verifyOnOTPVerificationPage(), "Incognito mode error message does not appear or valid user is not redirected to OTP verification page");
    }
}