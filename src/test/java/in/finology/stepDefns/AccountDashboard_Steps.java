...
//    }
    ...
    public void userIsLoggedInAndOnAccountDashboardPage() {
        assertTrue( ObjectManager.getObject().accountDashboardPage.verifyOnDashboardPage() && ObjectManager.getObject().accountDashboardPage.userPhotoDashboardBtnIsDisplayed(), "User is not logged in");
    }

    @Then("User should be redirected to account dashboard page")
    public void user_should_be_redirected_to_account_dashboard_page() {
        assertTrue(ObjectManager.getObject().accountDashboardPage.verifyOnDashboardPage(), "User is not on Account Dashboard page");
    }
    ...
    public void user_should_perform_logout_from_account_dashboard_page() {
        assertTrue(ObjectManager.getObject().accountDashboardPage.performLogoutFromAccountDashboardPageUsingSideBarLogoutBtn(), "User is unable to logout from the side bar logout button");
    }
    ...
    public void allDashboardElementsShouldBeDisplayed() {
        assertTrue(ObjectManager.getObject().accountDashboardPage.verifyAllElementsAreDisplayed(), "All elements in account dashboard page are not visible");
    }

    @When("User clicks user profile")
    public void userClicksUserProfile() {
        assertTrue(ObjectManager.getObject().accountDashboardPage.clickUserPhotoImgBtn(), "User is unable to logout from account dashboard page");
    }

    @Then("All user photo elements should be displayed")
    public void allUserPhotoElementsShouldBeDisplayed() {
        assertTrue(ObjectManager.getObject().accountDashboardPage.verifyAllUserPhotoElementsAreDisplayed(), "All elements of user photo elements in account dashboard page are not visible");
    }

    @Then("Welcome {string} should be displayed")
    public void welcomeShouldBeDisplayed(String message) {
        assertTrue(compareString(ObjectManager.getObject().accountDashboardPage.getWelcomeText(), message), "Welcome message of account dashboard page does not match");
    }
}