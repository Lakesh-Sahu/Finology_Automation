...
#      | Reriseerrrrrrrrrrrrrr    | reriserrrrrrrrrrwrrrrruserEmailMe2122@gmail.com | 6975957041    | Signup is unsupported in incognito. Please try in normal mode. |
  ...
  Scenario Outline: Unsuccessful sign up with invalid name, valid email id and valid mobile number
    ...
    Examples:
      ...
      | Rajq$User12  | randomNewEmailMe2123@gmail.com | 1257465983    | Your name can contain only alphabets. |

  @invalid_name_signup
  Scenario Outline: Unsuccessful sign up with empty name, valid email id and valid mobile number
    Given User enters "<name>", "<email id>" and "<mobile number>"
    When User clicks sign up button
...
#   Examples data are Sign Up manually due to OTP verification and then passed here (applying with different combinations)
